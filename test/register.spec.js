...
describe('Request Id', () => {
  ...
  it('Lack of input data: firstName', (done) => {
    agent.post('/api/auth/register')
      .type('form')
      .send({ email: faker.random.alphaNumeric(20), password: faker.random.alphaNumeric(10), firstName: '', lastName: faker.random.word() })
      .expect(422)
      .end((err, res) => {
        expect(err).to.not.exist;
        expect(res.body.error).to.equal('You must enter your full name.');
        done();
      });
  });
  ...
  it('Lack of input data: lastName', (done) => {
    agent.post('/api/auth/register')
      .type('form')
      .send({ email: faker.random.alphaNumeric(20), password: faker.random.alphaNumeric(10), firstName: faker.random.word(), lastName: '' })
      .expect(422)
      .end((err, res) => {
        expect(err).to.not.exist;
        expect(res.body.error).to.equal('You must enter your full name.');
